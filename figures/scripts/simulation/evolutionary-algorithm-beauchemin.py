...
nChild = int( sys.argv[2] )
nInd = int( sys.argv[3] )
nGen = int( sys.argv[4] )
tfreeStart = float( sys.argv[5] )
vfreeStart = float( sys.argv[6] )
tpauseStart = float( sys.argv[7] )
...
""" Function to run the R script at given parameters.
...
def get_fitness( parms ):
	...
	vfree = parms[2]
	tpause = parms[3]
	argstring = str(tfree) + " " + str(vfree) + " " + str(tpause) + " " + str(id)
	commandstring = "Rscript " + simulationScript + " " + argstring
	response = muterun( commandstring )
	if response.exitcode == 0:
		fitness = float( response.stdout.decode("utf-8") )
	else:
		print( response.stdout.decode("utf-8"))
		raise NameError("error in node: " + response.stderr.decode("utf-8"))
	d = pd.DataFrame( [[ g, id, tfree, vfree, tpause, fitness ]] )
	return d
...
def init_pop():
    pop = pd.DataFrame()
    pop = pd.concat( [pop, pd.DataFrame([[ tfreeStart, vfreeStart, tpauseStart ]]) ], ignore_index = True, axis = 1 )
...
def expand_pop(population):
    for ind in range(0,nInd):
        ...
        #population = population.append( ind_data, ignore_index=True )
    return population
...
def shrink_pop(population):
	...
	# fitnessData cols: 0 = gen, 1 = individual id (seed), 2 = tfree, 3 = vfree, 4 = tpause, 5 = fitness
	fitnessData = fitnessData.sort_values( by = 5, ascending = False )
